let most_common_char_in_list list =
  ...
  List.iter
    (fun char ->
      ...
      else Hashtbl.add hash_table char 1)
    list;
  Hashtbl.fold
    (fun char count acc -> if count > snd acc then (char, count) else acc)
    hash_table (' ', 0)
  |> fst
