...
let bot_rule_of_string low_to high_to = Some { low_to; high_to }

let parsers =
  [
    Utils.Input.parse "value %d goes to bot %d" (fun value bot_id ->
        ValueSetting { value; bot_id });
...
let string_of_rule rule =
  match rule with
  | Some { low_to; high_to } ->
      Printf.sprintf "low to %s, high to %s"
        (string_of_bot_or_output low_to)
        (string_of_bot_or_output high_to)
  | None -> "no rule"
...
let string_of_instruction inst =
  match inst with
  | ValueSetting { value; bot_id } ->
      Printf.sprintf "Value %d goes to bot %d" value bot_id
  | BotRule { bot_id; rule } -> (
      match rule with
      | Some { low_to; high_to } ->
          Printf.sprintf "Bot %d gives low to %s and high to %s" bot_id
            (string_of_bot_or_output low_to)
            (string_of_bot_or_output high_to)
      ...
      | ValueSetting { bot_id; value } ->
          if List.mem_assoc bot_id bots then
            ...
            if List.length new_values = 2 then
              if
                List.mem target_low new_values
                && List.mem target_high new_values
              then Some bot_id
              else
...
(* Simulate and stop when output_targets wehave all the outputs we care about *)
let rec process_bots_outputs bots rules values outputs output_targets =
  let have_targets =
    List.for_all (fun id -> List.mem_assoc id outputs) output_targets
  in
  if have_targets then Some outputs
  else
    match values with
    | [] -> None
    | v :: rest -> (
        match v with
        | ValueSetting { bot_id; value } ->
            if List.mem_assoc bot_id bots then (* exsits *)
              let current_values = List.assoc bot_id bots in
              let new_values = List.sort compare (value :: current_values) in
              if List.length new_values = 2 then
                let rule = find_rule bot_id rules in
                ...
                | Some { low_to; high_to } ->
                    ...
                    let new_outputs =
                      let tmp_outputs =
                        match low_to with
                        (* remove any existing output with this id and add the new value *)
                        | Output id -> (id, low) :: List.remove_assoc id outputs
                        | Bot _ -> outputs
                      in
                      match high_to with
                      (* remove any existing output with this id and add the new value *)
                      | Output id ->
                          (id, high) :: List.remove_assoc id tmp_outputs
                      | Bot _ -> tmp_outputs
                    in
                    let new_bots = List.remove_assoc bot_id bots in
                    process_bots_outputs new_bots rules
                      (new_instructions @ rest) new_outputs output_targets
                | _ ->
                    process_bots_outputs bots rules rest outputs output_targets
              else
                let new_bots = List.remove_assoc bot_id bots in
                let new_bot = (bot_id, new_values) in
                process_bots_outputs (new_bot :: new_bots) rules rest outputs
                  output_targets
            else
              let new_bot = (bot_id, [ value ]) in
              process_bots_outputs (new_bot :: bots) rules rest outputs
                output_targets
        | _ -> process_bots_outputs bots rules rest outputs output_targets)
...
let part1_impl filename low high =
  ...
  match bot_id with
  | Some bot_id ->
      Printf.printf "Part 1: Bot %d is the one we're looking for\n" bot_id
  | None -> Printf.printf "No bot found\n"

let part1 filename = part1_impl filename 17 61

let part2 filename =
  let output_targets = [ 0; 1; 2 ] in
  let instructions =
    filename
    |> Utils.Input.read_file_to_string
    |> Utils.Input.split_on_newline
  ...
  in
  let initial_bots = init_bots instructions in
  match process_bots_outputs initial_bots rules values [] output_targets with
  | Some outputs ->
      Printf.printf "Part 2: %d\n"
        (List.fold_left
           (fun acc (id, value) ->
             if List.mem id output_targets then acc * value else acc)
           1 outputs)
  | None -> Printf.printf "No solution for part 2\n"
