module Day05_impl = struct
    ...
    let rec aux acc remaining = 
      ...
      | curr :: next :: rest ->
        if curr.stop >= next.start then
          let merged = {start = curr.start; stop = max curr.stop next.stop} in
          aux acc (merged :: rest)
        else
          aux (curr :: acc) (next :: rest)
      | [curr] -> curr :: acc
    in
    aux [] sorted
  ...
  let part1 filename =
    ...
    in
    let ranges = parsed
    |> List.filter (function | Range _ -> true | _ -> false)
    |> List.map (function | Range x -> x | _ -> failwith "Expected range")
  in
  let available = parsed
  |> List.filter (function | Ingredient _ -> true | _ -> false)
  |> List.map (function | Ingredient x -> x | _ -> failwith "Expected ingredient")
  in
  let fresh_ingredients = available
  |> List.filter (
    fun ingredient -> is_fresh ingredient ranges
  )
  in
