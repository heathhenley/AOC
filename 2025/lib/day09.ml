module Day09_impl = struct
  let tile_of_line line =
    String.split_on_char ',' line |> List.map (fun s -> int_of_string s)
    |> fun x ->
    match x with
    | [ x; y ] -> (x, y)
  ...
  let poly_edges poly = List.combine poly (List.tl poly @ [ List.hd poly ])
  ...
  *)
  ...
  let part2 filename =
    let tiles =
      ...
      |> List.map tile_of_line
    in
    let rectangles = sorted_rectangles_of_pairs tiles in
    ...
    let rec find_best_rect candidates =
      ...
      | (d, t1, t2) :: tl ->
          ...
          if intersects br poly_edges then find_best_rect tl else d
    in
